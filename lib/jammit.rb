...
module Jammit
  ...
  JAVASCRIPT_COMPRESSORS        = [:yui, :closure, :uglifier]

  DEFAULT_JAVASCRIPT_COMPRESSOR = :yui

  CSS_COMPRESSORS               = [:yui, :sass]
  ...
  class << self
    attr_reader   :configuration, :template_function, :template_namespace,
                  ...
                  :template_extension_matcher, :allow_debugging, :public_root,
                  :compress_css

    attr_accessor :javascript_compressors
    attr_accessor :css_compressors
  end
  ...
  def self.load_configuration(config_path, soft=false)
    ...
    @compress_css           = conf[:compress_css].nil? ? @compress_assets : !(conf[:compress_css] == false)
